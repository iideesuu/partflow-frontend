...
const EXTERNAL_NUMBERING_ENDPOINT = '/numbering/external-register';
...
export function NewPartPage({navigate, user, openPart}) {
  ...
  const [numberSources, setNumberSources] = useState([]);
  const [major, setMajor] = useState('');
  const [categoryId, setCategoryId] = useState('');
  const [numberMode, setNumberMode] = useState('external');
  const [serial, setSerial] = useState('');
  const [externalSourceId, setExternalSourceId] = useState('');
  const [externalRequestId, setExternalRequestId] = useState('');
  const [externalPartCode, setExternalPartCode] = useState('');
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    Promise.all([apiRequest(endpoints.categories), apiRequest(endpoints.units), apiRequest(EXTERNAL_NUMBERING_ENDPOINT)])
      .then(([cats, unitData, numbering]) => {
        if (alive) {
          setCategories(list(cats)); setUnits(list(unitData));
          const sources = list(numbering?.sources || numbering);
          setNumberSources(sources);
          if (!externalSourceId && sources[0]?.id) setExternalSourceId(String(sources[0].id));
        }
      })
  ...
  const registered = numberRequests[externalRequestId];
  const partCode = numberMode === 'external'
    ? (registered?.part_code || externalPartCode.trim())
    : `${category?.major_code || '__'}${category?.minor_code || '__'}-${serial || '_____'}`;
  const ready = category && (numberMode === 'external'
    ? Boolean(registered?.id && registered.part_code) : /^[0-9]{5}$/.test(serial) && serial !== '00000');

  async function registerExternalNumber() {
    if (!category || !canEdit || reserving || registered) return;
    const expectedPrefix = `${category.major_code || ''}${category.minor_code || ''}`;
    if (!externalSourceId || !externalRequestId.trim() || !/^\d{4}-\d{5}$/.test(externalPartCode.trim()) || externalPartCode.endsWith('-00000') || !externalPartCode.startsWith(`${expectedPrefix}-`)) {
      setError('请选择外部编号源，并填写申请单号和符合 NNNN-NNNNN 格式的外部物料号。');
      return;
    }
    setReserving(true);
    setError('');
    const id = externalRequestId.trim();
    operationKeys.current[id] ||= crypto.randomUUID();
    try {
      const result = await apiRequest(EXTERNAL_NUMBERING_ENDPOINT, {
        method: 'POST',
        body: {source_id: externalSourceId, source_request_id: id, part_code: externalPartCode.trim(), operation_key: operationKeys.current[id]},
        idempotencyKey: operationKeys.current[id],
      });
      const code = result.returned_part_code || result.part_code;
      if (!result.id || !code) throw new Error('外部编号登记响应缺少物料号，请重新尝试。');
      setNumberRequests(current => ({...current, [id]: {id: result.id, part_code: code}}));
    ...
    try {
      ...
      const result = await apiRequest(endpoints.parts, {
        ...
        body: {
          ...
          ...(numberMode === 'external' ? {number_request: registered.id} : {}),
        },
      });
      setCreated(result);
      ...
      <form onSubmit={submit}>
        <section className="card form-card">
          ...
          <div className="form-grid">
            ...
            {numberMode === 'external' ? <>
              <Field label="外部编号源 *">
                <select required disabled={!canEdit || saving || reserving} value={externalSourceId}
                  onChange={event => { setExternalSourceId(event.target.value); setExternalRequestId(''); setExternalPartCode(''); }}>
                  <option value="">请选择外部编号源</option>
                  {numberSources.map(source => <option key={source.id} value={source.id}>{source.name}</option>)}
                </select>
              </Field>
              <Field label="外部申请单号 *">
                <input required maxLength={150} disabled={!canEdit || saving || reserving || Boolean(registered)} value={externalRequestId}
                  onChange={event => setExternalRequestId(event.target.value)} placeholder="例如 ERP-2026-0001" />
              </Field>
              <Field label="外部物料号 *">
                <input required maxLength={32} disabled={!canEdit || saving || reserving || Boolean(registered)} value={externalPartCode}
                  onChange={event => setExternalPartCode(event.target.value.trim())} placeholder="NNNN-NNNNN" />
              </Field>
              <Field label="编号登记">
                <button type="button" className="btn secondary" disabled={!category || !canEdit || saving || reserving || Boolean(registered)} onClick={registerExternalNumber}>
